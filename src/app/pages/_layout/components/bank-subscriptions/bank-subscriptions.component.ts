...
@Component({
  selector: 'app-bank-subscriptions',
  templateUrl: './bank-subscriptions.component.html',
  styleUrls: ['./bank-subscriptions.component.scss']
})
export class BankSubscriptionsComponent implements OnInit {

  vodSubsObj:any;
  p: number = 1;

  displayedColumns: string[] = [ 'id','payment_date','amount','student_id','student','teacher_id','teacher','created_at','status','action'];
  dataSource:any;
  ...
  constructor(private modalService: NgbModal, private fb: FormBuilder,private route: ActivatedRoute, private bankSubscriptionService: BankSubscriptionService,private cdr: ChangeDetectorRef,private toastr: ToastrService) { }
  ...
  getData(filter:any,limit: number,skip: number){
    this.bankSubscriptionService.getSltSubscription(filter,limit,skip).subscribe((list:any) => {
