...
export class ContactUsManagementComponent implements OnInit {
  ...
  displayedColumns: string[] = [ 'id','name','email','phone','subject','status','created_at','action'];
  dataSource:any;
  ...
  constructor(private modalService: NgbModal, private fb: FormBuilder, private route: ActivatedRoute, private contactUsService: ContactUsService,private cdr: ChangeDetectorRef,private toastr: ToastrService) { }
  ...
  getData(filter:any,limit: number,skip: number){
    this.contactUsService.getAllContactUsDetails(filter,limit,skip).subscribe((list:any) => {
      this.contactUsObj = list.data.results;
      this.pagination_data.total = list.data.pagination.total;
      this.dataSource = this.contactUsObj;
  ...
  addNewContactUs() {
    const modalRef = this.modalService.open(ContactUsModalComponent, { size: 'lg' });
  ...
  delete(id:any,name:string){
    ...
    modalRef.componentInstance.id = id;
    modalRef.componentInstance.type = 'Contact Us';
