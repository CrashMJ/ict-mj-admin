...
import { SltSubscriptionService } from './services/sltSubscription.service';
import { ToastrService } from 'ngx-toastr';

@Component({
  selector: 'app-slt-subscriptions',
  templateUrl: './slt-subscriptions.component.html',
  styleUrls: ['./slt-subscriptions.component.scss']
})
export class SltSubscriptionsComponent implements OnInit {

  vodSubsObj:any;
  p: number = 1;

  displayedColumns: string[] = [ 'id','transaction_date','transaction_amount','smart_id','transaction_id','sucess','created_at','subscription_ID','action'];
  dataSource:any;
  ...
  constructor(private modalService: NgbModal, private fb: FormBuilder,private route: ActivatedRoute, private sltSubscriptionService: SltSubscriptionService,private cdr: ChangeDetectorRef,private toastr: ToastrService) { }
  ...
  getData(filter:any,limit: number,skip: number){
    this.sltSubscriptionService.getSltSubscription(filter,limit,skip).subscribe((list:any) => {
